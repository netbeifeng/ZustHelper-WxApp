...
Page({
  ...
  onLoad: function () {
    ...
    app.globalData.shadeDegree = wx.getStorageSync('shadeDegree') || .3
    // wx.getStorage({
    //   key: 'shadeDegree',
    //   success: res => {
    ...
    //   },
    // })
   app.globalData.optimization = wx.getStorageSync('optimization') || false
    // wx.getStorage({
    //   key: 'optimization',
    //   success: res => {
    ...
    wx.request({
      url: app.globalData.requestHost + '/degree',
      method: 'GET',
      success: res => {
        if (res.data.code === 201) {
          app.globalData.degree = res.data.result
        }
      }
    })

    wx.request({
      url: app.globalData.requestHost + '/notice',
      method: 'GET',
      success: res => {
        if (res.data.code === 201) {
    ...
    wx.request({
      url: app.globalData.requestHost + '/share',
      method: 'GET',
      success: res => {
        if (res.data.code === 201) {
          app.globalData.share = res.data.result
        }
      }
    })
  ...
  resetConfig() {
    ...
    wx.setNavigationBarTitle({
      title: "科院数独 难度："+this.degreeTitle
    })
  },
  ...
  tapAvatar() {
    ...
    if (this.avatarShowTimes > 3) {
      this.tapAvatarShowTitle('数独 Fork. ZUST', 0, 1500, null)
      return
    }
  ...
  longTapAvatarTitle(e) {
    ...
    if (step === 8 || step === 9) {
      this.tapAvatarShowTitle('谢谢o(*￣▽￣*)ブ', 10, 0, res => {
        this.tapAvatarShowTitle('浙科院小助手', 0, 0, null)
      })
    }
  },
  /**
  ...
  onShareAppMessage: function (e) {
    ...
    if (e.from === 'button') {
      ...
      title = (share && share.range[range]) || '我在[ 浙科院小助手 - 数独小游戏 ]完成了一项数独挑战，成绩如下：'
      // console.log(img)
    } else {
